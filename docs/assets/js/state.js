...
const state = {
  ...
  building: {
    ...
    gebaeudetyp: '',
    anzahlWohnungen: '1',
    anzahlHeizungsanlagen: '1',
    heizzweck: 'Heizung + Warmwasser',
  ...
  return {
    ...
    consumption: {
      ...
      periods: state.consumption.periods.map((p) => ({
        ...
        consumption3: p.consumption3 || '',
        consumption4: p.consumption4 || '',
        vacancy: p.vacancy,
        warmWater: p.warmWater,
        climateFactor: p.climateFactor,
      })),
      extraPlants: (state.consumption.extraPlants || []).map((plant) => ({
        energietraeger: plant.energietraeger || '',
        unit: plant.unit || '',
        baujahr: plant.baujahr || '',
      })),
